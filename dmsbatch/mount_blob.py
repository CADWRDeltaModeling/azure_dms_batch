...
@click.option(
    "--storage-account-name", required=True, help="Name of the storage account"
)
@click.option("--container-name", required=True, help="Name of the container")
@click.option(
    "--mount-point", default="/mnt/resource/blobs", help="Mount point for the container"
)
@click.option(
    "--read-only",
    is_flag=True,
    default=False,
    help="Mount the container in read-only mode",
)
@click.option(
    "--cache-size-mb",
    default=1024,
    help="Size of the cache (mounted at {mount-point}/.tmp{container-name}) in MB",
)
def mount_blob(
    resource_group_name,
    storage_account_name,
    container_name,
    mount_point="/mnt/resource/blobs",
    read_only=False,
    cache_size_mb=1024,
):
    """make sure az cli is logged in to the correct subscription.
    ...
    resource = package.joinpath("blob_mount_config_template.yml")
    ...
    try:
        ...
        subprocess.check_output(
            f"mkdir -p {mount_point}/.tmp{container_name}", shell=True
        )
        subprocess.check_output(f"mkdir -p {mount_point}/{container_name}", shell=True)
        subprocess.check_output(
            f"blobfuse2 mount {mount_point}/{container_name} --config-file={tmp_config_fname}",
            shell=True,
        )
        print(
            f"Blob container {container_name} mounted at {mount_point}/{container_name}"
        )
    finally:
